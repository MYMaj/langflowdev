...
if [ "$ENVIRONMENT" = "development" ]; then
    ...
    exec python -m uvicorn --factory langflow.main:create_app --host 192.168.30.82 --port 7860 --log-level ${LOG_LEVEL:-info} --workers 2 --reload
else
    echo "Starting backend in production mode"
    exec langflow run --host 192.168.30.82 --port 7860 --log-level ${LOG_LEVEL:-info} --workers -1 --backend-only
fi
